...
-- Materials manufacturer ERP - table definitions
-- ============================================
...
-- Organization master (self-referencing through PARENT_ORG_ID)
...
CREATE TABLE INV_INVENTORY (
    ...
    -- Derby only allows UNIQUE over NOT NULL columns, so "no lot" is the empty string
    LOT_ID           VARCHAR(100) DEFAULT '' NOT NULL,
    ...
    -- Stops a second stock row for the same product x warehouse x lot, which would
    -- silently break every SUM(QUANTITY)
    CONSTRAINT UQ_INV_INVENTORY_STOCK UNIQUE (PRODUCT_ID, WAREHOUSE_ID, LOT_ID)
);
...
CREATE TABLE INV_STOCK_MOVEMENTS (
    ...
    IS_ACTIVE        VARCHAR(10) DEFAULT '1',
    CREATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- Matches INV_RECEIPTS / INV_SHIPMENTS. FROM is null on a receipt, TO on a shipment
    FOREIGN KEY (FROM_WAREHOUSE_ID) REFERENCES INV_WAREHOUSES(WAREHOUSE_ID),
    FOREIGN KEY (TO_WAREHOUSE_ID) REFERENCES INV_WAREHOUSES(WAREHOUSE_ID)
);
