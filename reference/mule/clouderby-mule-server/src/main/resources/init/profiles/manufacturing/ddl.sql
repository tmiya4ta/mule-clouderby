...
CREATE TABLE INV_INVENTORY (
    ...
    -- Derby の UNIQUE 制約は NOT NULL 列にしか張れないため、ロット無しは空文字で表す
    ...
    -- 同一 品目 x 倉庫 x ロット の在庫行が二重に立つのを防ぐ (SUM(QUANTITY) が壊れる)
...
CREATE TABLE INV_STOCK_MOVEMENTS (
    ...
    IS_ACTIVE        VARCHAR(10) DEFAULT '1',
    CREATED_AT       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- INV_RECEIPTS / INV_SHIPMENTS と揃える。入庫は FROM が、出庫は TO が NULL
